...
define('PDO_DSN','mysql:host=127.0.0.1;dbname=test');
...
class PdoOrmTest extends TestCase {
	...
	function testCreate(){
		...
		$re1 = $orm -> exec("drop table if exists phpec_unittest");
	...
	function testQueryException($orm){
		$orm -> exec("delete from not_exists_table");
	}
	/**
	 *@depends testCreate
	 *@expectedException  Exception
	 *@expectedExceptionMessage PHPec PDOOrm Error: placeholder not match
	 */
	function testQueryNotMatch($orm){
		$orm -> exec("select * from phpec_unittest where id=? or id=?", 1);
	}
	/**
	 *@depends testCreate
	 */
	function testQuery($orm){
		//insert
		$re = $orm -> exec("insert into phpec_unittest set name=?,author=?,nums=?",['day day up','Tim',22]);
		$this -> assertEquals(1,$re);
		//insert another
		$orm -> exec("insert into phpec_unittest set name=?,author=?,nums=?",['good good study','Kate',2]);
		//get
		$re = $orm -> exec("select * from phpec_unittest where author=?",'Tim');
		$this -> assertEquals('day day up',$re[0]['name']);
		$this -> assertEquals('Tim',$re[0]['author']);
		$this -> assertEquals(22,$re[0]['nums']);
		$re = $orm -> exec("select * from phpec_unittest where author=?",'Kate');
		$this -> assertEquals('good good study',$re[0]['name']);
		//update
		$re = $orm -> exec("update phpec_unittest set author=? where id=?",['Kitty',$re[0]['id']]);
		$this -> assertEquals(1,$re);
		$re = $orm -> exec("update phpec_unittest set author=? where id=?",['Kitty',$re[0]['id']]);
		$this -> assertEquals(0,$re);
		$re = $orm -> exec("select * from phpec_unittest where author=?",'Kate');
		$this -> assertEquals([],$re);
		$re = $orm -> exec("select * from phpec_unittest where author=?",'Kitty');
		$this -> assertEquals('good good study',$re[0]['name']);
		$re = $orm -> exec("delete from phpec_unittest where id>?", 0);
		$this -> assertEquals(2,$re);
		$re = $orm -> exec("select count(*) as num from phpec_unittest");
	...
	function testMap($orm){
		...
		$this -> assertTrue($obj instanceof \PHPec\PdoDao);
		return $obj;
	}
	/**
	 *@depends testMap
